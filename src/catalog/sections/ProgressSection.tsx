...
const STYLE_ROWS: StyleRow[] = [
  { prop: "color", value: "brand",   cssClass: ".brand",   properties: ["background: var(--brand-primary)"]   },
  { prop: "color", value: "success", cssClass: ".success", properties: ["background: var(--success-primary)"] },
  { prop: "color", value: "warning", cssClass: ".warning", properties: ["background: var(--warning-primary)"] },
  { prop: "color", value: "error",   cssClass: ".error",   properties: ["background: var(--error-primary)"]   },
  { prop: "size",  value: "sm", cssClass: ".sm", properties: ["height: 4px"] },
  { prop: "size",  value: "md", cssClass: ".md", properties: ["height: 8px"] },
];
...
type ActiveState = { color: ProgressColor; size: ProgressSize };

function isActive({ prop, value }: StyleRow, state: ActiveState): boolean {
  if (prop === "color") return value === state.color;
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          ...
          <tbody>
            <tr>
              ...
              <td style={TD}>
                {["background: var(--surface-tertiary)", "border-radius: var(--radius-round)", "overflow: hidden"].map(p => (
            ...
            <tr>
              <td style={{ ...TD, whiteSpace: "nowrap" }}>
                <code style={{ fontSize: "12px", fontFamily: "monospace", color: "#18181B", backgroundColor: "#F4F4F5", padding: "2px 6px", borderRadius: "4px" }}>.fill</code>
              </td>
              <td style={TD}>
                {["transition: width 300ms ease"].map(p => (
                  <div key={p} style={{ fontSize: "12px", fontFamily: "monospace", color: "#52525B", lineHeight: "1.9" }}>{p}</div>
                ))}
              </td>
            </tr>
...
type PlaygroundProps = {
  ...
  color: ProgressColor; onColor: (c: ProgressColor) => void;
  size:  ProgressSize;  onSize:  (s: ProgressSize)  => void;
};

function Playground({ value, onValue, color, onColor, size, onSize }: PlaygroundProps) {
  const [copied, setCopied] = useState(false);
  const [codeOpen, setCodeOpen] = useState(false);
  const snippet = `<Progress value={${value}} color="${color}" size="${size}" />`;
        ...
        controls={
          <>
            ...
            <ControlRow label="Color">
              {COLORS.map(c => <Pill key={c} active={color === c} onClick={() => onColor(c)}>{c}</Pill>)}
            </ControlRow>
      ...
      <div style={{ display: "flex", gap: "32px", alignItems: "flex-start", marginBottom: "8px" }}>
        <div style={{ flex: "0 0 52%", minWidth: 0 }}>
          <SectionBlock title="Playground">
            <Playground value={value} onValue={setValue} color={color} onColor={setColor} size={size} onSize={setSize} />
          </SectionBlock>
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <SectionBlock title="Style Reference">
            <StyleReference color={color} size={size} />
          </SectionBlock>
        </div>
      </div>
