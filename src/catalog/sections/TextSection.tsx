...
const ROLES: TypographyRole[] = [
  "headline-regular", "headline-bold",
  "title-regular",    "title-bold",
  "medium-regular",   "medium-bold",
  "body-regular",     "body-bold",
  "label-regular",    "label-bold",    "label-caps",
  "xs-regular",       "xs-bold",
];

const ROLE_GROUPS: { label: string; roles: TypographyRole[] }[] = [
  { label: "Headline", roles: ["headline-regular", "headline-bold"] },
  { label: "Title",    roles: ["title-regular",    "title-bold"]    },
  { label: "Medium",   roles: ["medium-regular",   "medium-bold"]   },
  { label: "Body",     roles: ["body-regular",     "body-bold"]     },
  { label: "Label",    roles: ["label-regular",    "label-bold",    "label-caps"] },
  { label: "XS",       roles: ["xs-regular",       "xs-bold"]       },
];
...
const STYLE_ROWS: StyleRow[] = [
  ...ROLES.map(role => ({
    ...
    value:      role,
    cssClass:   `.${role}`,
    properties: getRoleProperties(role),
  })),
...
function getRoleProperties(role: TypographyRole): string[] {
  ...
  const scale = role.split("-")[0];
  const weight = role.endsWith("bold") ? "var(--font-weight-bold)" : "var(--font-weight-regular)";
  const props = [
    `font: ${sizeMap[scale]}`,
    `font-weight: ${weight}`,
  ];
  if (role === "label-caps") {
    ...
    props.push("letter-spacing: var(--letter-spacing-caps)");
  }
  return props;
}
        ...
        controls={
          <>
            {ROLE_GROUPS.map(group => (
              <ControlRow key={group.label} label={group.label}>
                {group.roles.map(r => (
                  <Pill key={r} active={role === r} onClick={() => onRole(r)}>
                    {r.replace(`${r.split("-")[0]}-`, "")}
                  </Pill>
                ))}
              </ControlRow>
...
export function TextSection() {
  const [role,  setRole]  = useState<TypographyRole>("body-regular");
    ...
    <SplitPage files={sources}>
      <div style={{ marginBottom: "32px" }}>
        ...
        <p style={{ margin: 0, fontSize: "15px", color: "#52525B", lineHeight: "1.6", maxWidth: "600px" }}>
          Renders any text using a named role. Each role locks in font-size, line-height, font-weight, and the correct HTML tag.
          Never apply font styles manually - always use <code style={{ backgroundColor: "#F4F4F5", padding: "1px 5px", borderRadius: "3px", fontSize: "13px" }}>{"<Text role=\"...\">"}</code>.
        </p>
      </div>
