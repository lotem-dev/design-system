...
const STYLE_ROWS: StyleRow[] = [
  { prop: "size", value: "xs", cssClass: ".xs", properties: ["width: 20px", "height: 20px", "font-size: 8px"] },
  { prop: "size", value: "sm", cssClass: ".sm", properties: ["width: 24px", "height: 24px", "font-size: 9px"] },
  { prop: "size", value: "md", cssClass: ".md", properties: ["width: 32px", "height: 32px", "font-size: 11px"] },
  { prop: "size", value: "lg", cssClass: ".lg", properties: ["width: 40px", "height: 40px", "font-size: 14px"] },
];
...
function Playground({ size, onSize }: PlaygroundProps) {
  ...
  const snippet = `<Avatar initials="JD" size="${size}" />`;
...
function StyleReference({ size }: { size: AvatarSize }) {
  return (
    <div>
      {/* Base styles */}
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          ...
          <tbody>
            {STYLE_GROUPS.map(({ prop, rows }) =>
              rows.map((row, i) => {
                const active = prop === "size" && row.value === size;
                return (
